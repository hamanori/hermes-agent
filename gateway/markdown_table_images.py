...
def render_table_pngs(table_markdown: str, output_dir: str | Path | None = None) -> list[str]:
    """Render a Markdown table block to one or more PNG files.
    ...
    font = _load_font(ImageFont, bold=True, size=30)
    header_font = _load_font(ImageFont, bold=True, size=31)
    padding_x = 20
    outer_padding = 24
    max_table_width = _int_env("HERMES_DISCORD_TABLE_IMAGE_MAX_WIDTH", 1100, minimum=640, maximum=1600)
    min_col_width = 110
    max_col_width = 360
    ...
    max_body_rows = _int_env("HERMES_DISCORD_TABLE_IMAGE_MAX_BODY_ROWS", 5, minimum=4, maximum=80)
    if len(rows) <= max_body_rows:
        chunks = [rows]
    else:
        chunks = [rows[index:index + max_body_rows] for index in range(0, len(rows), max_body_rows)]
    blank_row = [""] * len(headers)
    chunks[-1] = chunks[-1] + [blank_row] * (max_body_rows - len(chunks[-1]))
    ...
    for part_index, chunk in enumerate(chunks, start=1):
        paths.append(
            _render_table_png(
                ...
                show_header=part_index == 1,
                col_widths=col_widths,
            )
        )
    if len(paths) > 1:
        _normalize_image_dimensions(paths)
    return paths


def render_table_png(table_markdown: str, output_dir: str | Path | None = None) -> str:
    """Render a Markdown table block to the first PNG file and return its path."""
    return render_table_pngs(table_markdown, output_dir)[0]
...
def _render_table_png(
    ...
    *,
    ...
    all_rows = ([headers] if show_header else []) + rows
    font = _load_font(ImageFont, bold=True, size=30)
    header_font = _load_font(ImageFont, bold=True, size=31)
    padding_x = 20
    padding_y = 15
    ...
    for row_index, row in enumerate(all_rows):
        row_font = header_font if show_header and row_index == 0 else font
    ...
    guide_x = outer_padding
    for col_width in col_widths[:-1]:
        guide_x += col_width
        draw.line(
            [(guide_x, outer_padding), (guide_x, height - outer_padding)],
            fill=(255, 255, 255, 32),
            width=1,
        )
    ...
    for row_index, wrapped_row in enumerate(wrapped_rows):
        ...
        for col_index, lines in enumerate(wrapped_row):
            ...
            for line in lines:
                ...
                align = "left" if show_header and row_index == 0 else alignments[col_index]
                if align == "right":
                    ...
                    text_x = x + padding_x
                draw.text((text_x, text_y), line, font=row_font, fill="#ffffff", stroke_width=1, stroke_fill="#ffffff")
                text_y += _line_height(draw, row_font) + line_gap
            x += col_width
        if row_index < len(wrapped_rows) - 1:
            line_alpha = 120 if show_header and row_index == 0 else 48
            draw.line(
    ...
    digest = hashlib.sha1(source_markdown.encode("utf-8")).hexdigest()[:10]
    suffix = f"-part{part_index:02d}of{part_count:02d}" if part_count > 1 else ""
    path = out_dir / f"table-{int(time.time() * 1000)}-{digest}{suffix}.png"
...
def _calculate_column_widths(
    ...
    *,
    ...
    return [max(min_col_width, int(width * scale)) for width in natural_widths]


def _normalize_image_dimensions(paths: Sequence[str]) -> None:
    from PIL import Image

    images = []
    try:
        for path in paths:
            images.append((path, Image.open(path).convert("RGBA")))
        max_width = max(image.width for _, image in images)
        max_height = max(image.height for _, image in images)
        for path, image in images:
            if image.width == max_width and image.height == max_height:
                continue
            normalized = Image.new("RGBA", (max_width, max_height), (0, 0, 0, 0))
            normalized.alpha_composite(image, (0, 0))
...
def segment_markdown_tables(
    ...
    *,
    ...
    renderer: Callable[[str, str | Path | None], str | Sequence[str]] | None = None,
    ...
    render = renderer or render_table_pngs
    ...
    for start, end, table_markdown in _iter_table_blocks(text):
        ...
        try:
            rendered = render(table_markdown, output_dir)
            paths = [rendered] if isinstance(rendered, str) else list(rendered)
            if not paths:
                raise RuntimeError("renderer returned no image paths")
            for path in paths:
